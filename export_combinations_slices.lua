...
dlg:entry {
    ...
    text = "{spritename}_{layerpair}_{slicename}"
}
dlg:combobox {
    id = 'format',
    label = 'Export Format:',
...
for _, slice in ipairs(Sprite.slices) do
    ...
    slice_filename = slice_filename:gsub("{spritename}",
                         RemoveExtension(Basename(Sprite.filename)))
    ...
    if not string.find(slice_filename, "{layerpair}") then
        local dlg = MsgDialog("Error", "{layerpair} must be in the format string.")
        dlg:show()
        return 1
    end
    slice_count[slice_id] = slice_count[slice_id] - 1
